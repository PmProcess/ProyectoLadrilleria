...
<body>
    <h3 style="text-transform: uppercase;text-align:center">Reporte de Ventas</h3>
    ...
    <table style="width: 100%;margin-top:10px;" cellspacing=0>
        ...
        <tbody>
            @foreach ($datos as $fila)
                ...
                @forelse ($fila['productos'] as $producto)
                    @if ($loop->first)
                    ...
                    @if ($loop->last)
                        <tr>
                            <th style="border-bottom: 2px solid red!important;" colspan="2">{{ $producto->nombre }}
                            </th>
                            <th style="border-bottom: 2px solid red!important;">{{'S/.'. number_format($producto->total,2) }}</th>
                        </tr>
                    @else
                    <tr>
                        <th colspan="2">{{ $producto->nombre }}
                        </th>
                        <th>{{ 'S/.' .number_format( $producto->total,2) }}</th>
                    </tr>
                    @endif
                @empty
                    {{-- <tr>
